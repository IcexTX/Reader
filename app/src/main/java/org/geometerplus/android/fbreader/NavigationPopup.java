...
final class NavigationPopup extends ZLApplication.PopupPanel {
    ...
    private void createPanel(FBReader activity, RelativeLayout root) {
        ...
        final TextView previous_chapter = (TextView) myWindow.findViewById(R.id.previous_chapter);
        final TextView next_chapter = (TextView) myWindow.findViewById(R.id.next_chapter);
        ...
        final Button but_increase_font = (Button) myWindow.findViewById(R.id.but_increase_font);
        final Button but_decrease_font = (Button) myWindow.findViewById(R.id.but_decrease_font);
        View.OnClickListener listener = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                } else if (v == previous_chapter) {
                    switchChapter(false);
                } else if (v == next_chapter) {
                    switchChapter(true);
                } else if (v == but_increase_font) {
                    setFontSize(+2);
                } else if (v == but_decrease_font) {
                    setFontSize(-2);
                }
        ...
        previous_chapter.setOnClickListener(listener);
        next_chapter.setOnClickListener(listener);
        btnOk.setOnClickListener(listener);
        btnCancel.setOnClickListener(listener);
        but_increase_font.setOnClickListener(listener);
        but_decrease_font.setOnClickListener(listener);
        ...
        but_increase_font.setText(resource.getResource("increaseFont").getValue());
        but_decrease_font.setText(resource.getResource("decreaseFont").getValue());
    }
    ...
    private void switchChapter(boolean isNext) {
        ...
        Root = myFBReader.Model.TOCTree;
        myItems = new ZLTree[Root.getSize() - 1];
        myOpenItems.clear();
        myOpenItems.add(Root);
        int curpage = selectItem(tocElement);
        if (curpage == Root.getSize()) {
            return;
        }
        final TOCTree tree = (TOCTree) getItem(isNext ? curpage + 1 : curpage - 1);
        final TOCTree.Reference reference = tree.getReference();
        if (reference != null) {
            myFBReader.addInvisibleBookmark();
            myFBReader.BookTextView.gotoPosition(reference.ParagraphIndex, 0, 0);
    ...
    public void setFontSize(int size) {
        final ZLIntegerRangeOption option = myFBReader.ViewOptions.getTextStyleCollection().getBaseStyle().FontSizeOption;
        //最小字体为10
        if (option.getValue() == 10) {
            return;
        }
        option.setValue(option.getValue() + size);
        myFBReader.clearTextCaches();
        myFBReader.getViewWidget().repaint();
    }

    public final ZLTree<?> getItem(int position) {
        final int index = indexByPosition(position, Root) - 1;
