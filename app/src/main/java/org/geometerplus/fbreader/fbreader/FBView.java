...
public final class FBView extends ZLTextView {
    ...
    private void onFingerSingleTapLastResort(int x, int y) {
        myReader.runAction(getZoneMap().getActionByCoordinates(x, y, getContextWidth(), getContextHeight(),
    ...
    private abstract class Footer implements FooterArea {
        ...
        protected int getBatteryLevel() {
            ...
            if (footerOptions.ShowBattery.getValue()) {
                return myReader.getBatteryLevel();
            } else {
                return 100;
            }
        }
    ...
    private class FooterNewStyle extends Footer {
        ...
        public synchronized void paint(ZLPaintContext context) {
            ...
            final int left = 40;
            final int height = getHeight();
            final int charHeight = setFont(context, 30, height > 12);
            final int marginLeft = charHeight * 2 + charHeight * 2 / 12 + left + 20;
            //final int right = context.getWidth() - getRightMargin();
            final int right = context.getWidth() - 40;
            final PagePosition pagePosition = FBView.this.pagePosition();

            //电池
            context.setBatteryColor(textColor);
            context.drawBattery(left, (height + charHeight + 1) / 2, charHeight, getBatteryLevel());
            //时间
            final String time = buildInfoString("  ");
            context.setTextColor(textColor);
            context.drawString(marginLeft, (height + charHeight + 1) / 2, time);
