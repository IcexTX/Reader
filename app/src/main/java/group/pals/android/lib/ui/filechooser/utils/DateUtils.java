...
public class DateUtils {
     ...
     * example: "10:01 AM".
     */
    public static final int _FormatShortTime = android.text.format.DateUtils.FORMAT_12HOUR | android.text.format.DateUtils.FORMAT_SHOW_TIME;

    /**
     * Used with format methods of {@link android.text.format.DateUtils}. For
     * example: "Oct 01".
     */
    public static final int _FormatMonthAndDay = android.text.format.DateUtils.FORMAT_ABBREV_MONTH | android.text.format.DateUtils.FORMAT_SHOW_DATE | android.text.format.DateUtils.FORMAT_NO_YEAR;
    ...
    public static String formatDate(Context context, Calendar date, FileTimeDisplay fileTimeDisplay) {
        ...
        if (android.text.format.DateUtils.isToday(date.getTimeInMillis())) {
            ...
            if (fileTimeDisplay.isShowTimeForOldDaysThisYear()) {
                res = android.text.format.DateUtils.formatDateTime(context, date.getTimeInMillis(), _FormatShortTime | _FormatMonthAndDay);
            }else {
            ...
            if (fileTimeDisplay.isShowTimeForOldDays()) {
                res = android.text.format.DateUtils.formatDateTime(context, date.getTimeInMillis(), _FormatShortTime | _FormatMonthAndDay | _FormatYear);
            }else {
                res = android.text.format.DateUtils.formatDateTime(context, date.getTimeInMillis(), _FormatMonthAndDay | _FormatYear);
            }
